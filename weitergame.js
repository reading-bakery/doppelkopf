document.addEventListener('DOMContentLoaded', () => {
    ...
    // Globale Variablen für die aktuelle Session
    ...
    // --- Kern-Funktionen ---
    function openModal(spielerArray, aktuelleRunde) {
        aktuellesSpielerArray = spielerArray;
        aktuelleRundenNummer = Number(aktuelleRunde) || 0;
        
        renderSoloOptions(spielerArray);
        renderTeamOptions(spielerArray);
    ...
    const berechnePunkte = () => {
        ...
        aktuellesSpielerArray.forEach(spieler => {
    ...
    gamesList.addEventListener('click', (e) => {
        if (e.target.classList.contains('btn-continue')) {
            ...
            const gameData = JSON.parse(card.dataset.gameInfo);
            window.activeGameForContinue = gameData;
            openModal(gameData.spielerArray, gameData.runde);
        }
    });
    ...
    gameModal.addEventListener('click', (e) => {
        ...
        if (e.target.id === 'final-save-btn') {
            ...
            // Runde erhöhen
            aktuelleRundenNummer += 1;

            let bodyParts = [];
            bodyParts.push(`${entryIds.spiel_datum}=${encodeURIComponent(game.datum)}`);
            bodyParts.push(`${entryIds.punkte_gesamt}=${encodeURIComponent(game.neuePunkte || document.getElementById('modal-points-input').value)}`);
            bodyParts.push(`${entryIds.aktuelle_runde}=${encodeURIComponent(aktuelleRundenNummer)}`);
            
            aktuellesSpielerArray.forEach((name, i) => bodyParts.push(`${entryIds[`s${i+1}_name`]}=${encodeURIComponent(name)}`));
            aktuellesSpielerArray.forEach((name) => bodyParts.push(`${entryIds[`s${Object.keys(ergebnisse).indexOf(name)+1}_punkte`]}=${encodeURIComponent(ergebnisse[name])}`));
            aktuellesSpielerArray.forEach((name, i) => bodyParts.push(`${entryIds[`s${i+1}_team`]}=${encodeURIComponent(teamRe.includes(name) ? "Re" : "Kontra")}`));
